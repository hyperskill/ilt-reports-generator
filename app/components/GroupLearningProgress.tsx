...
export function GroupLearningProgress({ 
  reportId,
  students, 
  submissions, 
  structure, 
  courseId, 
  meetings 
}: GroupLearningProgressProps) {
  const [moduleProgress, setModuleProgress] = useState<ModuleProgress[]>([]);
  const [loading, setLoading] = useState(true);
  const [expandedRows, setExpandedRows] = useState<Set<number>>(new Set());

  useEffect(() => {
    loadGroupLearningData();
  }, [reportId, students, submissions, structure, courseId, meetings]);

  const loadGroupLearningData = async () => {
    try {
      setLoading(true);
      ...
      const moduleAverages = new Map<number, ModuleProgress>();
      ...
      const finalProgress = Array.from(moduleAverages.values())
        .map(avg => ({
          ...avg,
      ...
      console.error('Error loading group learning data:', err);
      setModuleProgress([]);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <Card>
      <Heading size="5" mb="3">🎯 Learning Outcomes & Tools Progress</Heading>
      <Text size="2" color="gray" mb="4">
        Track how the group is mastering learning outcomes and tools across course modules.
      </Text>
          ...
          <Table.Root variant="surface">
            ...
            <Table.Body>
              {modulesWithContent.map((module) => {
                ...
                const progressColor = getProgressColor(module.avg_completion_rate);
                    ...
                    <Table.Cell>
                      <Flex direction="column" gap="1">
                        <Badge color={progressColor} size="1">
                          {module.avg_completion_rate.toFixed(0)}% done
                        </Badge>
                        <Text size="1" color="gray">
                          {module.avg_success_rate.toFixed(0)}% success
                        </Text>
                        <Text size="1" color="gray">
      ...
      <Box mt="4" p="3" style={{ backgroundColor: 'var(--gray-a2)', borderRadius: 'var(--radius-2)' }}>
        <Flex gap="4" wrap="wrap">
          ...
          <Box>
            <Text size="1" color="gray" as="div">Avg Group Progress</Text>
            <Text size="3" weight="bold" style={{ 
              color: `var(--${getProgressColor(
                moduleProgress.reduce((sum, m) => sum + m.avg_completion_rate, 0) / moduleProgress.length
              )}-11)` 
            }}>
              {(moduleProgress.reduce((sum, m) => sum + m.avg_completion_rate, 0) / moduleProgress.length).toFixed(1)}%
            </Text>
          </Box>
        </Flex>
      </Box>
          ...
          <Accordion.Item value="help" style={{ border: '1px solid var(--gray-6)', borderRadius: '6px' }}>
            ...
            >
              <div style={{ fontSize: '14px', lineHeight: '1.7' }} className="help-content">
                <p><strong>What this shows:</strong> This table connects your course's educational goals with actual student progress. It shows which learning outcomes and tools students are mastering in each module.</p>
                
                <p><strong>Table columns explained:</strong></p>
                <ul>
                  <li><strong>Module</strong> - The course section name and position</li>
                  <li><strong>Progress</strong> - Completion rate (color-coded), success rate, and student count
                    <ul>
                      <li>🟢 Green badge: ≥75% completion (excellent)</li>
                      <li>🟠 Orange badge: 50-74% completion (moderate)</li>
                      <li>🔴 Red badge: &lt;50% completion (needs attention)</li>
                    </ul>
                  </li>
                  <li><strong>Learning Outcomes</strong> - All educational objectives for this module</li>
                  <li><strong>Tools</strong> - Technologies and platforms students practice with</li>
                </ul>
                
                <p><strong>What to look for:</strong></p>
                <ul>
                  <li><strong>High completion + high success</strong> - Students are mastering outcomes effectively</li>
                  <li><strong>High completion + low success</strong> - Students are engaged but struggling with content</li>
                  <li><strong>Low completion</strong> - Module may need restructuring or additional support</li>
                  <li><strong>"Not defined"</strong> - Visit Settings to generate outcomes/tools for better tracking</li>
                </ul>
              </div>
              <style jsx>{`
